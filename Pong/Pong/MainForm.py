...
class MainForm(Form):
    ...
    def TimerballTick(self, sender, e):
        ...
        if ball.Location.X <= 0 or ball.Location.X < lpdl.Left - 10:
               """ TODO: FINISH LEFT BOUNDARY """
               pass
           
        if ball.Location.X >= self.Width or ball.Location.X > rpdl.Right + 10:
               lscore += 1
    ...
    def MainFormKeyDown(self, sender, e):
        ...
        if e.KeyCode == Keys.T: 
            bl.BackColor = Color.Violet
        if e.KeyCode == Keys.T: 
            lpdl.BackColor = Color.Red
        if e.KeyCode == Keys.T: 
            rpdl.BackColor = Color.RoyalBlue
        if e.KeyCode == Keys.T: 
